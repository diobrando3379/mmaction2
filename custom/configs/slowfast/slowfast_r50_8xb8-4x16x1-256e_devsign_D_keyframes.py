...
ann_file_train = 'custom/data/devsign_D_keyframes/train_split.txt'  # 相对路径
ann_file_val   = 'custom/data/devsign_D_keyframes/val_split.txt'
ann_file_test  = 'custom/data/devsign_D_keyframes/test_split.txt'

# 若txt里用绝对路径，在此留空；若相对路径，在此写根目录
data_root = '/mnt/e/Dataset/CV_Dataset/DEVISIGN/DEVISIGN_D_keyframes'
data_root_val = '/mnt/e/Dataset/CV_Dataset/DEVISIGN/DEVISIGN_D_keyframes'
data_root_test = '/mnt/e/Dataset/CV_Dataset/DEVISIGN/DEVISIGN_D_keyframes'
NUM_CLASSES = 500
...
train_pipeline = [
    dict(type='SampleFrames', clip_len=32, frame_interval=1, num_clips=1), # 一个样本一共才40帧，间隔2会超出
...
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        filename_tmpl='color_keyframe_{:06}.jpg', # 按你的实际命名修改
...
val_dataloader = dict(
    ...
    dataset=dict(
        ...
        filename_tmpl='color_keyframe_{:06}.jpg',
        start_index=1,
...
param_scheduler = [
    dict(type='LinearLR', start_factor=0.0001, by_epoch=True, begin=0, end=10, convert_to_iter_based=True),
    dict(type='CosineAnnealingLR', T_max=246, eta_min=1e-6, by_epoch=True, begin=10, end=256)
]
